...
export const getFile = async ({ type, data }: GetFileOptions) => {
  return await match(type)
    ...
    .with(DocumentDataType.S3_PATH, async () => getFileUrl(data))
    .exhaustive();
};
...
export const getFileUrl = async (key: string) => {
  try {
    ...
    const getPresignedUrlResponse = await fetch(`${baseUrl}/api/files/presigned-get-url`, {
      method: 'POST',
      headers: {
    ...
    console.error(`Error fetching presigned URL for key "${key}":`, error);
    throw new Error(`Failed to get presigned url for key "${key}"`);
  }
};
