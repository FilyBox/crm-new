...
export type GetStatsInputChat = {
  user: Pick<User, 'id' | 'email'>;
  where?: Prisma.DocumentWhereInput;
  team: GetTeamCountsOption;
...
export const getStatsChat = async ({
  ...
  const [ownerCounts, notSignedCounts, hasSignedCounts] = await getTeamCounts({
    ...options.team,
    createdAt,
  ...
  notSignedCounts.forEach((stat) => {
    ...
    if (stat.status === ExtendedDocumentStatus.PROCESSING) {
      stats[ExtendedDocumentStatus.PROCESSING] += stat._count._all;
    }
  });
