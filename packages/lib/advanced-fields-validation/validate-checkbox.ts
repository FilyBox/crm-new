...
export const validateCheckboxField = (
  ...
  if (readOnly && required) {
    errors.push(t`A field cannot be both read-only and required`);
  }

  if (values.length === 0) {
    errors.push(t`At least one option must be added`);
  }

  if (readOnly && values.length === 0) {
    errors.push(t`A read-only field must have at least one value`);
  }

  if (isSigningPage && required && values.length === 0) {
    errors.push(t`Selecting an option is required`);
  }

  if (validationRule && !validationLength) {
    errors.push(t`You need to specify the number of options for validation`);
  }

  if (validationLength && !validationRule) {
    errors.push(t`You need to specify the validation rule`);
  }
  ...
  if (validationRule && validationLength) {
    ...
    if (validation) {
      ...
      if (lengthCondition) {
        let errorMessage;
        if (isSigningPage) {
          errorMessage = t`You need to ${validationRule.toLowerCase()} ${validationLength} options`;
        } else {
          errorMessage =
            validation.value === '<='
              ? t`You need to select at least ${validationLength} options`
              : t`You need to add at least ${validationLength} options`;
        }
